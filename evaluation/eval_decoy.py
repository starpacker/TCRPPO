...
def load_decoys(decoy_library_root, target_sequences,
    ...
    """Load and merge decoys from all four Decoy A/B/C/D tiers (GLOBAL version).

    DEPRECATED: Use load_decoys_for_target() for per-target evaluation.

    This function loads ALL decoys from all targets merged together.
    Only Decoy C (global cross-reactive library) should be shared across targets.
    Decoy A/B/D should be loaded per-target using load_decoys_for_target().
    """
    out = []
    ...
    if include_a:
        out.extend(_load_decoy_a_all(decoy_library_root))
    if include_b:
        out.extend(_load_decoy_b_all(decoy_library_root))
    if include_d:
...
def load_decoys_for_target(decoy_library_root, target_sequence,
                           include_a=True, include_b=True, include_c=True, include_d=True,
                           decoy_c_cache=None):
    """Load decoys for a SPECIFIC target peptide.

    This is the CORRECT way to load decoys for evaluation:
    - Decoy A: Load only from data/decoy_a/<TARGET>/
    - Decoy B: Load only from data/decoy_b/<TARGET>/
    - Decoy C: Global library (shared, can pass cache)
    - Decoy D: Per-target cross-reactive peptides

    Args:
        decoy_library_root: Path to decoy library
        target_sequence: The specific target peptide sequence
        include_a/b/c/d: Toggle for each tier
        decoy_c_cache: If provided, use this instead of re-loading Decoy C

    Returns:
        list of decoy dicts, deduped by (peptide, hla)
    """
    out = []

    # Decoy C is global - use cache if available
    if include_c:
        if decoy_c_cache is not None:
            out.extend(decoy_c_cache)
        else:
            out.extend(_load_decoy_c(decoy_library_root))

    # Decoy A - per target
    if include_a:
        out.extend(_load_decoy_a_for_target(decoy_library_root, target_sequence))

    # Decoy B - per target
    if include_b:
        out.extend(_load_decoy_b_for_target(decoy_library_root, target_sequence))

    # Decoy D - per target (already correct)
    if include_d:
        out.extend(_load_decoy_d(decoy_library_root, target_sequence))
...
def _load_decoy_a_all(root):
    """Load ALL Decoy A entries (from all target subdirectories). DEPRECATED."""
    ...
    print("[load] Decoy A (all): {} entries".format(len(out)))
    return out


def _load_decoy_a_for_target(root, target_sequence):
    """Load Decoy A entries for a SPECIFIC target peptide."""
    base_dir = os.path.join(root, "data", "decoy_a", target_sequence)
    path = os.path.join(base_dir, "decoy_a_results.json")
    if not os.path.isfile(path):
        return []
    out = []
    with io.open(path, encoding="utf-8") as f:
    ...
    for e in items:
        out.append({
            ...
            "tier": "A",
            "evidence_level": "",
            "origin_target": e.get("target_sequence", target_sequence),
        })
    return out


def _load_decoy_b_all(root):
    """Load ALL Decoy B entries (from all target subdirectories). DEPRECATED."""
    ...
    print("[load] Decoy B (all): {} entries".format(len(out)))
    return out


def _load_decoy_b_for_target(root, target_sequence):
    """Load Decoy B entries for a SPECIFIC target peptide."""
    base_dir = os.path.join(root, "data", "decoy_b", target_sequence)

    # Try final_ranked_decoys.json first, then decoy_b_results.json
    path = os.path.join(base_dir, "final_ranked_decoys.json")
    if not os.path.isfile(path):
        path = os.path.join(base_dir, "decoy_b_results.json")
        if not os.path.isfile(path):
            return []
...
def main():
    ...
    # --- Pre-load Decoy C (global, shared across all targets) ------------
    decoy_c_cache = None
    if not args.no_decoy_c:
        decoy_c_cache = _load_decoy_c(args.decoy_library_root)

    # --- Print per-target decoy counts for verification ------------------
    print("\n[load] Per-target decoy counts:")
    for tgt in target_seqs:
        decoys_for_target = load_decoys_for_target(
            args.decoy_library_root, tgt,
            include_a=not args.no_decoy_a,
            include_b=not args.no_decoy_b,
            include_c=not args.no_decoy_c,
            include_d=not args.no_decoy_d,
            decoy_c_cache=decoy_c_cache,
        )
        tier_counts = {}
        for d in decoys_for_target:
            tier_counts[d["tier"]] = tier_counts.get(d["tier"], 0) + 1
        print("  {}: {} decoys {}".format(tgt, len(decoys_for_target), tier_counts))
    ...
    with io.open(args.out_csv, "w", encoding="utf-8", newline="") as f:
        ...
        for ti, target_meta in enumerate(targets):
            ...
            # Load decoys SPECIFIC to this target (correct per-target evaluation)
            decoys = load_decoys_for_target(
                args.decoy_library_root, target_seq,
                include_a=not args.no_decoy_a,
                include_b=not args.no_decoy_b,
                include_c=not args.no_decoy_c,
                include_d=not args.no_decoy_d,
            ...
            for d in decoys:
                tier_counts[d["tier"]] = tier_counts.get(d["tier"], 0) + 1
            print("  loaded {} decoys for this target: {}".format(len(decoys), tier_counts))
