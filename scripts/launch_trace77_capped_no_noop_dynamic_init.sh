#!/bin/bash
# Launch trace77: capped no-noop dynamic init curriculum.

set -e

cd /share/liuyutian/tcrppo_v2

RUN_NAME="trace77_capped_no_noop_dynamic_init"
CONFIG="configs/trace77_capped_no_noop_dynamic_init.yaml"
...
echo "=========================================="
echo "trace77: capped no-noop dynamic initial-affinity curriculum"
echo "=========================================="
...
echo ""
echo "Preparing independent trace77 tFold cache..."
for suffix in "" "-wal" "-shm"; do
    src="data/tfold_feature_cache_trace73_curriculum_exploration.db${suffix}"
    dst="data/tfold_feature_cache_trace77_capped_no_noop_dynamic_init.db${suffix}"
    if [ -f "$src" ] && [ ! -f "$dst" ]; then
        cp -f "$src" "$dst"
    fi
done
