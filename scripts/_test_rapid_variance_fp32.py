#!/usr/bin/env python3
"""Quick test: score the SAME 50 pairs through FP32 tFold server N times rapidly.
Compare with BF16 AMP results to determine if AMP causes drift."""
...
print(flush=True)
print("=" * 60, flush=True)
print("FP32 RAPID-FIRE VARIANCE ANALYSIS", flush=True)
print("=" * 60, flush=True)
...
if np.std(means) < 0.05:
    print(f"\nVERDICT: DETERMINISTIC (std of means = {np.std(means):.4f} < 0.05)", flush=True)
...
# Compare with BF16 results
print(flush=True)
print("=" * 60, flush=True)
print("COMPARISON: FP32 vs BF16 AMP", flush=True)
print("=" * 60, flush=True)
try:
    with open("logs/tfold_rapid_fire_variance.json") as f:
        bf16_data = json.load(f)
    bf16_means = bf16_data["means"]
    print(f"  BF16 AMP means:  {[f'{m:.4f}' for m in bf16_means]}", flush=True)
    print(f"  BF16 std:        {np.std(bf16_means):.4f}", flush=True)
    print(f"  BF16 spread:     {max(bf16_means)-min(bf16_means):.4f}", flush=True)
    print(flush=True)
    print(f"  FP32 means:      {[f'{m:.4f}' for m in means]}", flush=True)
    print(f"  FP32 std:        {np.std(means):.4f}", flush=True)
    print(f"  FP32 spread:     {max(means)-min(means):.4f}", flush=True)
    print(flush=True)
    if np.std(means) < np.std(bf16_means) / 2:
        print(f"  → FP32 is MUCH MORE STABLE than BF16 AMP", flush=True)
    elif np.std(means) < np.std(bf16_means):
        print(f"  → FP32 is MORE STABLE than BF16 AMP", flush=True)
    else:
        print(f"  → FP32 drift is SIMILAR to BF16 AMP (not an AMP issue)", flush=True)
except FileNotFoundError:
    print("  (BF16 data not found for comparison)", flush=True)

# Save
with open("logs/tfold_rapid_fire_variance_fp32.json", "w") as f:
    json.dump({"means": means, "all_scores": all_scores, "per_pair_std": per_pair_std.tolist()}, f, indent=2)
print(f"\nSaved to logs/tfold_rapid_fire_variance_fp32.json", flush=True)
