#!/usr/bin/env python
"""Re-evaluate top models with tFold scorer and merge results with existing ERGO evaluations.

This script:
1. Loads existing ERGO evaluation results from results/<model>_eval/eval_results.json
2. Re-evaluates the same generated TCRs using tFold scorer
...
Usage:
    python scripts/reevaluate_with_tfold.py --models test41,test14,test33,test39
"""
...
def evaluate_with_tfold(
    ...
    """Evaluate TCRs with tFold scorer, matching ERGO evaluation format."""
    import time
    ...
    print(f"  Evaluating {n_tcrs} TCRs for {peptide} with tFold...")
    ...
    # Use batch scoring to reduce server load
    # First, score all targets in batch
    print(f"    Scoring {n_tcrs} targets in batches...")
    target_scores = []
    batch_size = 1  # Process one at a time to avoid overwhelming server
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(f"      Retry {attempt+1}/{max_retries} for target batch {i//batch_size + 1}")
                    time.sleep(5)
                else:
                    print(f"      ERROR: Failed target batch after {max_retries} attempts, using 0.5")
                    target_scores.extend([0.5] * len(batch_tcrs))

        if (i + batch_size) % 100 == 0:
            print(f"      Progress: {min(i+batch_size, n_tcrs)}/{n_tcrs} targets")
        time.sleep(1)  # Small delay between batches

    # Then score all decoys for each TCR
    print(f"    Scoring decoys ({len(all_decoys)} per TCR)...")
    per_tcr_decoy_scores = []
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        decoy_scores.extend([0.5] * len(batch_decoys))
            time.sleep(0.5)  # Small delay between decoy batches
...
def main():
    ...
    parser.add_argument("--models", type=str, required=True, help="Comma-separated model names (e.g., test41,test14)")
    ...
    # Increase max_subprocess_batch to handle larger batches
    tfold_scorer.max_subprocess_batch = 8
    ...
    for model_name in models:
        ...
        for peptide, target_data in data.items():
            ...
            # Get all TCRs (in original order, not ranked)
            tcrs = [item["tcr"] for item in ergo_data["per_tcr_ranked"]]

            # Evaluate with tFold
            tfold_results = evaluate_with_tfold(tcrs, peptide, tfold_scorer)
