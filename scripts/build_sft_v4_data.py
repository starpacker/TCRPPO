#!/usr/bin/env python3
"""
Build SFT v4 dataset: Pure imitation learning with deterministic action order.

Key differences from v2:
1. Position order is deterministic (left-to-right, not random shuffle)
2. This makes imitation learning easier - same state always leads to same action
"""
...
def generate_init_tcr_v4(final_tcr: str, n_subs: int) -> Tuple[str, List[Dict]]:
    """Generate init TCR with DETERMINISTIC action order (left-to-right).

    Returns:
        (init_tcr, actions) where actions are ordered by position (low to high).
    """
    L = len(final_tcr)
    if n_subs > L:
        n_subs = L
    ...
    # CHANGED: Sort positions left-to-right (deterministic order)
    sorted_positions = sorted(mutations.keys())
...
def build_dataset(
    ...
    """Build SUB-only SFT v4 dataset with deterministic action order."""
    random.seed(seed)
    ...
    # Collect all peptide/hla associations per final TCR
    ...
    # Step 3: Generate trajectories with deterministic order
    trajectories = []
    n_verified = 0
    n_failed = 0
    ...
    for tcr, info in finals.items():
        ...
        for i in range(n_inits_per_final):
            ...
            init_tcr, actions = generate_init_tcr_v4(tcr, n_subs)
            ...
            trajectories.append({
                ...
                'actions': actions,
                'reconstruction_method': 'reverse_augment_v4',
                'n_subs': n_subs,
            })

    print(f"Generated {len(trajectories)} trajectories (verified: {n_verified}, failed: {n_failed})")

    # Step 4: Also include original same-length pairs with deterministic order
    n_original = 0
    for p in pairs:
        ...
        positions = [pos for pos in range(len(init_tcr)) if init_tcr[pos] != final_tcr[pos]]

        if len(positions) == 0 or len(positions) > max_steps:
            continue

        # CHANGED: Sort positions (no shuffle)
        positions.sort()
        actions = []
        for pos in positions:
            actions.append({
        ...
        if verify_trajectory(init_tcr, final_tcr, actions):
            trajectories.append({
                ...
                'actions': actions,
                'reconstruction_method': 'original_sub_only_v4',
...
def main():
    parser = argparse.ArgumentParser(description="Build SFT v4 dataset (pure imitation)")
    parser.add_argument("--input", type=str,
                        default="/share/liuyutian/tcrppo_v2/data/sft_raw_pairs.json")
    parser.add_argument("--output", type=str,
                        default="/share/liuyutian/tcrppo_v2/data/sft_v4_trajectories.json")
    ...
    output_data = {
        'metadata': {
            'version': 'v4',
            'changes': 'Deterministic position order (left-to-right) for pure imitation learning',
            'n_inits_per_final': args.n_inits_per_final,
            'max_steps': args.max_steps,
            'seed': args.seed,
            'stats': stats,
