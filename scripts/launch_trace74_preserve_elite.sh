#!/bin/bash
# Launch trace74: preserve/polish high-affinity online seeds from trace73.

set -e

cd /share/liuyutian/tcrppo_v2

RUN_NAME="trace74_preserve_elite"
CONFIG="configs/trace74_preserve_elite.yaml"
...
echo "=========================================="
echo "trace74: preserve elite high-A seeds"
echo "=========================================="
...
echo ""
echo "Preparing independent trace74 tFold cache..."
for suffix in "" "-wal" "-shm"; do
    src="data/tfold_feature_cache_trace73_curriculum_exploration.db${suffix}"
    dst="data/tfold_feature_cache_trace74_preserve_elite.db${suffix}"
    if [ -f "$src" ] && [ ! -f "$dst" ]; then
        cp -f "$src" "$dst"
    fi
done
