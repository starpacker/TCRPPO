#!/bin/bash
# Launch trace76: dynamic init curriculum with no-op prevention.

set -e

cd /share/liuyutian/tcrppo_v2

RUN_NAME="trace76_no_noop_dynamic_init"
CONFIG="configs/trace76_no_noop_dynamic_init.yaml"
...
echo "=========================================="
echo "trace76: no-noop dynamic initial-affinity curriculum"
echo "=========================================="
...
echo ""
echo "Preparing independent trace76 tFold cache..."
for suffix in "" "-wal" "-shm"; do
    src="data/tfold_feature_cache_trace73_curriculum_exploration.db${suffix}"
    dst="data/tfold_feature_cache_trace76_no_noop_dynamic_init.db${suffix}"
    if [ -f "$src" ] && [ ! -f "$dst" ]; then
        cp -f "$src" "$dst"
    fi
done
