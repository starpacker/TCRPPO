#!/usr/bin/env python3
"""
Real-time monitoring for trace100 experiment
...
def monitor_log(logfile, window=200):
    ...
    try:
        ...
        # Compute statistics
        nat_pass_count = sum(1 for ep in episodes if ep['nat'] > -0.3)
        ...
        both_count = sum(1 for ep in episodes if ep['nat'] > -0.3 and ep['target_short'] < 0.5)
        ...
        print(f"")
        print(f"🎯 Key Metrics:")
        print(f"  Nat pass rate (>-0.3):        {nat_pass_rate:5.1f}%  ({nat_pass_count}/{len(episodes)})")
        ...
        if nat_pass_rate < 30:
            print(f"  → Naturalness needs improvement (target: >50%)")
        if target_good_rate < 50:
            print(f"  → Affinity needs improvement (target: >70%)")
...
if __name__ == '__main__':
    logfile = 'logs/trace100_hard_nat_gate_train.log'

    if len(sys.argv) > 1:
        logfile = sys.argv[1]

    print(f"trace100 Real-Time Monitor")
    print(f"Log: {logfile}")
