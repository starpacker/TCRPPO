#!/usr/bin/env python
"""Re-evaluate top models with tFold scorer - simple one-by-one version.

This script:
1. Loads existing ERGO evaluation results from results/<model>_eval/eval_results.json
2. Re-evaluates the same generated TCRs using tFold scorer (one at a time)
...
Usage:
    python scripts/reevaluate_with_tfold_simple.py --models test41 --device cuda
"""
...
def evaluate_with_tfold(
    ...
    """Evaluate TCRs with tFold scorer, one at a time with delays."""
    ...
    print(f"  Evaluating {n_tcrs} TCRs for {peptide} with tFold (slow mode)...")
    ...
    for i, tcr in enumerate(tcrs):
        ...
        for attempt in range(max_retries):
            try:
                target_score, _ = tfold_scorer.score(tcr, peptide)
                break
            except Exception as e:
                if attempt < max_retries - 1:
                    print(f"      Retry {attempt+1} for TCR {i} target: {str(e)[:50]}")
                    time.sleep(5)
                else:
                    print(f"      ERROR: Failed TCR {i} target after {max_retries} attempts")
                    target_score = 0.5
        target_scores.append(target_score)
        time.sleep(1)  # Delay after each target

        # Decoy scores
        decoy_scores = []
        for j, decoy_pep in enumerate(all_decoys):
            for attempt in range(max_retries):
                try:
                    decoy_score, _ = tfold_scorer.score(tcr, decoy_pep)
                    break
                except Exception as e:
                    if attempt < max_retries - 1:
                        time.sleep(3)
            ...
            time.sleep(0.5)  # Delay after each decoy
